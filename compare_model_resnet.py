...
if args.cmodel:
	...
	#model = load_compare_model(model)
...
for i in range(STEPS):
	...
	for pos_pair in pos_pair_examples:
		...
		cosine_loss = loss_fn(cosine_dis, ones)
		...
		loss.backward()
		optimizer.step()
		sys.stdout.write("Step: %d, total loss: %.4f, score loss: %.4f, cosine loss:%.4f\r" % 
	...
	for neg_pair in neg_pair_examples:
		...
		cosine_loss = loss_fn(cosine_dis, zeros)
		loss = score_loss + cosine_loss
		loss_list.append(loss.data[0])
		if len(loss_list) > 1000:
			loss_list.popleft()
...
central_crop = transforms.Compose([
	transforms.CenterCorp((224, 224))
])
